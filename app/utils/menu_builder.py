...
def build_menu(page: ft.Page):
    ...
    # Se non è loggato → menu minimale
    if user_name == "Utente" or user_role == "N/A":
        return ft.Container(
            content=ft.Column([
                ft.Text("Non autenticato", size=18, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                ft.Divider(height=10, color=ft.Colors.WHITE),
                ft.ElevatedButton("Vai al Login", on_click=lambda e: page.go("/"))
            ], spacing=10),
            bgcolor="#1e90ff",
            padding=20,
            width=200,
